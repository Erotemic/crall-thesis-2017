...
% TODO: relate back to GZC, and talk about non-rally scinareos
...
Assuming that each edge is correctly labeled and , 
...
Furthermore, we can construct a \glossterm{deterministic termination criteria}; if a negative edge is placed
...
\begin{itemln}
    \item PCCs with redundant edges are less likely to contain an errors.
...
Additionally, the deterministic termination criteria would require that each pair of PCCs has a redundant set of
  negative edges between them before the algorithm stop.
%$O(|V|^2)$ edges are reviewed as
%  negative.
...
%While this criteria will not guarantee ,
...
First we describe the redundancy criteria in~\cref{sec:redun}, and then cover inconsistency recovery
  in~\cref{sec:incon}.
...
Using this redundancy criteria we are able to find and remove edges from the priority queue that are no longer
  needed.
...
Therefore we choose candidate edges depending on what properties of the graph we want to manipulate.
...
These edges are assigned prioritized in the same way, however in this iteration the refresh criteria is disabled.
...
After we have ensure positive-redundancy the outer loop alternates back to generating candidate edges using the
  ranking algorithm.
In this way our algorithm alternates between two modes, first searching for merges, and then ensuring redundancy.
This continues until the termination criteria that we will describe in~\cref{sec:converge} is satisfied.
...
If the new edge causes an inconsistency we drop into inconsistency mode we will discuss in~\cref{sec:incon}.
...
In~\cref{sec:redun} we described a redundancy criteria that exposes errors by introducing inconsistencies.
...
If the edges required to complete the underlying real PCCs are missing from the candidate set, then nothing can
  be done.
We therefore must develop a ``refresh criteria'' to determine when to stop and update the current priority queue
...
To ensure that the identification is perfect, the algorithm would need to use all $\binom{|V|}{2}$ edges as
  candidates and then terminate using the deterministic convergence criteria explained in the introduction of this
  chapter.
Recall that the deterministic convergence criteria only stops the algorithm once each PCC is positive redundant
...
%In this example we use a window span of $s=20$, a patience of $a=20$, and a threshold of $t=\exp{-2}\approx.135$.
...
\section{Experiments}\label{sec:graphexpt}
    ...
    \item \pvar{rank+clf} --- the same ranking algorithm but augmented with the automatic classification algorithm from \cref{chap:pairclf}, and finally
    ...
    However, \pvar{ranking} and \pvar{rank+clf} must first be adapted with a protocol defining the order of
      reviews and when to stop.

    \subsection{Identification protocols}

    We design the protocol for \pvar{ranking} to be similar to the approach described in \cref{sec:introgzc} that
      was used in the \GZC{}.
    ...
    The protocol for \pvar{rank+clf} is only slightly modified from the used for the \pvar{ranking}.
    ...
    Finally, the protocol for \pvar{graph} has already been defined in ~\cref{sub:graphalgo}.
    ...
    The convergence criteria determines when candidate edges should be refreshed and when the algorithm should
      terminate.
    The automatic classifier is the same one used in \pvar{rank+clf}, but because this algorithm can recover from
    ...
    The number of remaining merges (on the left) slowly decreases \pvar{ranking}, which must manually review all
      result.
    For \pvar{graph} and \pvar{rank+clf} the initial decrease is instantaneous due to the automatic
      classification algorithm, which does not cost manually reviews, but once manual reviews begin note that the
      slope of \pvar{graph} is steeper than \pvar{rank+clf} due to the redundancy mechanisms that remove extraneous
      reviews.
    Furthermore, while \pvar{rank+clf} and \pvar{ranking} continue until their candidate edges are exhausted,
      \pvar{graph} is able to terminate once the curve asymptotes using the convergence criteria.
    ...
    To further analyze the errors in a simulation, we compare the predicted PCCs to the real ground-truth PCCs.
    ...
    In all cases we see that \pvar{graph} results in the most number of common PCCs and fewest splits due to its
    ...
    Lastly, many of the split cases for the \pvar{graph} algorithm involve splitting just one singleton
