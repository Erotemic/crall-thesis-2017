...
\section{Experiments}\label{sec:graphexpt}
    ...
    The procedure for \pvar{rank+clf} is only slightly modified from the used for the \pvar{ranking}.
    ...
    Finally, the procedure for \pvar{graph} has already been defined in ~\cref{sub:graphalgo}.
    To quickly recap, LNBNN is used to search for candidate edges, which are then assigned probabilities and
      inserted into a priority queue based on these probabilities.
    Candidate edges are either automatically or manually classified based on their order in the priority queue.
    ...
    For the convergence criterion use a patience of $a=20$, a window span of $s=20$, and a termination threshold
      of $\exp{-2}\approx0.135$, which causes the criterion to trigger after at most $20$ consecutive
      non-meaningful reviews.
    ...
    \Cref{fig:Simulation,fig:Refresh} shows these results for plains and Grévy's zebra datasets.
    ...
    The number of remaining merges (on the left) slowly decreases for \pvar{ranking}, which is the only algorithm
      that requires manual review of each pair.
    ...
    Furthermore, while \pvar{rank+clf} and \pvar{ranking} continue until their candidate edges are exhausted,
      while \pvar{graph} is able to terminate once the curve asymptotes using the convergence criterion.
    ...
    To further analyze the errors in the simulation, we compare the predicted PCCs to the real groundtruth PCCs
    ...
    \begin{enumln}
        \item The ``correct'' group consists of predicted PCCs that exactly match a real PCC.
        For a predicted PCC to be correct there must be a real PCC with the same nodes.
        
        \item the ``split'' predicted PCCs that must be split into real PCCs, and
      
        \item the ``merge'' predicted PCCs that must be merged into real PCCs.
    \end{enumln}
    For each group we count the number of predicted PCCs and the number of real PCCs they correspond to.
    We also measure the average number of annotations that belong to each of these predicted and real PCCs.
    ...
    Note that the majority of predicted PCCs can be purely split or merged into real PCCs, but there are some
      predicted PCCs that are counted twice because they must first be split and then merged before the groundtruth
      is achieved.
