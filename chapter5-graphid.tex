...
To generate candidate edges that may merge existing PCCs, we issue each annotation as a query to the ranking
  ...
  lists.
In addition, we can use the positive PCC augmentation algorithm from~\cref{subsec:augredun} to generate edges
  that would ensure all PCCs are positive redundant.
In practice, we alternate between these two modes in each iteration of the outer loop, first searching for
  merges, and then ensuring redundancy.
