\section{Instance recognition}\label{sec:ir}
    ...
    On one end of the spectrum, \glossterm{instance recognition} techniques --- like scene
    ...
    \subsection{Spatial verification}\label{subsec:sverreview}
        ...
        Inconsistent features are removed using \glossterm{spatial
          verification}, a process based on the random sample consensus
        ...
        %  outliers.
        When RANSAC returns a large enough set of inliers (\wrt{} some
          threshold) the hypothesis model it is generally considered to
          be a ``good fit''.
          %Typically, if RANSAC estimates more than a threshold
          %number of inliers it is said to be a good approximation of the
          %data.
    ...
    \subsection{Bag-of-words instance recognition}\label{subsec:bow}
        ...
        One of the most well known techniques in instance recognition
          is the \glossterm{bag-of-words} model introduced to computer
        ...
        The concept of a visual word is formalized using a visual
          vocabulary.
        A \glossterm{visual vocabulary} is defined by clustering
          feature descriptors using k-means~\cite{lloyd_least_1982}.
        The centroids of the clusters represent the \glossterm{visual
        ...
        %Finally, the matching database images are ranked and a shortlist of
        %  images are spatially verified.
        %The inlier correspondences are used to re-score and re-rank the database
        %  images.
        ...
        \paragraph{Formal bag-of-words scoring}
            ...
            %$n_{ci}$ be the number of occurrences of word $\c$ in image $i$,
            %$n_{i}$ be the number of words in image $i$, 
        ...
        \paragraph{Extensions to bag-of-words}
            ...
            Query Expansion is a way to increase the recall of
    ...
    \subsection{Fisher Vector}
        ...
        %\devcomment{TODO:
        %the Sanchez paper is actually about classification} 
    ...
    \subsection{VLAD --- Vector of Locally Aggregated Descriptors} 
        ...
        Recently, NetVLAD --- a convolutional variant of the VLAD
          descriptor --- has been
    ...
    \subsection{SMK --- the Selective Match Kernel}\label{sec:smk}
        ...
        In the SMK framework a matching function and selectivity
          function are chosen.
    ...
    \subsection{Face recognition and verification}
        ...
        Due to the specific nature of this problem specialized features
        ...
        One of the most recent advances in face verification and
    ...
    \subsection{Discussion --- instance recognition}
        ...
        We have found this quantization to cause a noticeable drop in
          performance.
        %A vocabulary based approach trades off accuracy and simplicity for
        %  scalability and efficiency.
        %Many of the descriptors in our application will be unique to a single
        %  individual, and some individuals may only be seen once.
        %Standard methods for learning a vocabulary will likely result in a
        %  sub-optimal partitioning of descriptor space and a reduction in
        %  accuracy.
        ...
        %All of our objects have been localized in their image, this reduces
        %  background matches and helps ensure that a similar number of features
        %  are detected in each object.
        %The accuracy of nearest neighbors is greatly improved when using
        %  multiple kd-trees \cite{silpaanan_optimised_2008} as opposed to the
        %  single kd-tree used in Lowe's algorithm.
        %Instead of using a hough-transform \cite{c_method_1962,
        %  grimson_object_1990} for spatial verification, we employ a deterministic
        %  version of RANSAC based on elliptical correspondences similar to
        %  \cite{philbin_object_2007, chum_homography_2012}.
        %We also use a different scoring mechanism based on a category
        %  recognition technique LNBNN \cite{mccann_local_2012}, which is be
        %  discussed~\cref{sec:lnbnn}.

        % chktex-file 8
        Even though we would prefer to retain the discriminative
        ...
        %Such work is an avenue for future research, but is beyond the scope of
        %  this \thesis{}.

        We will not evaluate hashing schemes such as min
          hash~\cite{chum_fast_2012} because we are more concerned about
          accuracy than speed.
        %In our application recall is much less important than having retrieved
        %  at least one highly ranked correct result.
        %In individual animal recognition returning
