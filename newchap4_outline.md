...
Learning the 1-v-1 classifiers
------------------------------
...
## Constructing the classifier
    ...
    * Missing information --- feature vectors will contain NaN values
...
# Posterior Assignment
    * We use multicut to determine the most likely partitioning of the
      annotations given the initial graph.
    * All probabilities are first converted into two state probabilities of
      {same, different} for each edge.
...
## Connected Component Review Algorithm:
    ...
    * Priority is based on matching probability. Non-matching edges are
       considered next, and not-comparable edges are considered last
       because they add the least amount of information.  Reviewing
...
## Redundant Edges for Error Detection
    ...
    * This algorithm uses only the unweighted distance between nodes to
      determine which redudnat edges should be added.
    * Future Work:
        * Each connected component is assigned a probability that no
          mistakes were made classifying any of the edges within a connected
          component.
        * Any component with a low probability is marked for review. 
        * A new edge that maximizes the probability increase is selected for review.
        * Likewise each pair of compoments with a negative review is assigned a
...
## Merge Case Detection
    * Merges are handled naturally by the system. These are detected as high
      probability edges between two annotations in different unreviewed
      connected compoments.
    * When two annotations are linked by a positive review their connected
      components merge.
    * A problem occurs when two annotations in different components are
      incorrectly marked as not-matching.
    * Redundant edges between two components already reviewed to be different 
      may result in an inconsistency, which allows us to detect a merge case
      error.
...
Bootstrapping classifiers for new species
-----------------------------------------
...
* Create covering sets based on probabilities for each annotation based on a threshold.
...
## Cross Validation
    * Split database into test/train sets using K-Folds. For each fold, the names
      in the test set are disjoint from the names in the train set.
...
## Evaluation
    * Each test predicts a partitioning --- a cut value of True or False for
    ...
    * From these we can compute binary classification metrics such as precision
      and recall.
...
## Evaluating Manual Partitioning
    ...
    * Using the priority order manually review edges until the priority queue
      is empty. 
    * In this test precision and accuracy are only dependant on the 1-v-M
      rankings, but the amount of user reviews will be large.

## Evaluating Automatic Partitioning
    * Thresholds: Link all annotations above a threshold. Do assign any
      negative labels to avoid inconsistent edges.
    * Multicut
    * Alpha Expansion
    * Report classification statistics to compare algorithms and provide a
      baseline.
    * In this test precision and accuracy depend entirely on the algorithms,
      but no user reviews are needed.
