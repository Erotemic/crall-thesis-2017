...
\label{tbl:SimDetails}
    \begin{subfigure}[h]{\textwidth}\centering\input{figures5/PZ_Master1/error_size.tex}\caption{Plains zebra}\end{subfigure}
    \begin{subfigure}[h]{\textwidth}\centering\input{figures5/GZ_Master1/error_size.tex}\caption{Grévy's zebra}\end{subfigure}
\end{table}
}
...
\begin{table}[b]
    \centering
    \caption[\caplbl{TestTrainDBStats}Database statistics for the graph simulation]{\caplbl{TestTrainDBStats}
    % ---
    Database statistics for the end-to-end graph simulation.
    We split each dataset into training and test sets.
    The training set is used to learn the pairwise classifiers, and we perform the simulation on the test set.
    We report the number of names, the number of annotations, and the average number of annotations per name for
      each set.
    For the training set we also report how many edges (pairs of annotations) were used to train the classifiers.
    % ---
    }
