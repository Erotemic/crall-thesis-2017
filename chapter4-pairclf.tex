...
\section{Pairwise classification experiments}\label{sec:pairexpt}
    ...
    We will compare these predicted match-state probabilities to the scores generated by LNBNN.
    ...
    In addition, we compare the match-state classifier to LNBNN in two ways.
    ...
    \subsection{Matching state}
        ...
        From these multiclass classifications we build a confusion matrix for plains and we Grévy's zebras.
        This confusion matrix is shown in \cref{tbl:ConfusionMatch}.
        ...
        FIXME: Need to rerun experiments and write based on that output.
        ...
        First, it is not surprising that incomparable pairs like the ones illustrated in \cref{fig:PairFailIN}
          might fail be labeled as incomparable because of the lack of incomparable training data.
        ...
        Interestingly inspecting the failure cases reveled several labeling errors in the database.
    ...
    \subsection{Photobomb state}
        ...
        For plains zebras the MCC of $0.24$ demonstrates that the photobomb classifier has weak but significant
          predictive power.
        For Grévy's zebras the MCC of $0.0$ suggests that this photobomb classifier has no predictive power.
        However, these classification metrics presuppose that the classification is the state with the maximum
          probability.
        In the two case scenario this means to be classified as a photobomb the probability must be greater than
          $0.5$.
        Because there is such a large training class imbalance, it will be difficult for the decision trees in
          the random forest to ensure that more than half of the training samples at a leaf node are photobombs.
        If we instead can choose a different threshold, then we see that the photobomb classifier is indeed
          learning.
        This is illustrated by the ROC curves in \cref{fig:PhotobombROC}, which plots the true positive and false
          positive rate as a function of an adjustable threshold.
        The AUC of $0.90$ for plains zebras and $0.95$ for Grévy's zebras are well above chance, and indicate
          that the pairwise feature vector does contain enough information to predict if a pair of annotations is a
          photobomb.

        Considering this, we we adjust the classification threshold of each photobomb-state classifier to
          maximize the MCC.
        We plot the MCC as a function of this threshold in~\cref{fig:PBThreshMCC}.
        We then find that we achieve an MCC of $0.34$ for plains zebras and $0.40$ for Grévy's zebras.
        While these numbers are not overwhelmingly strong, they are significant and better represent the true
          predictive power of the photobomb-state classifier.
        We report the confusion matrix and evaluation metrics corresponding to this adjusted threshold
          in~\cref{tbl:ConfusionPhotobomb,tbl:EvalMetricsPhotobomb}.

          %the quality of the photobomb-state classifier could be greatly improved with more training data.
        The top $10$ most important feature in~\cref{tbl:ImportantPBFeat} also support the conclusion that the
          photobomb-state classifier is learning.
        The photobomb classifier relies on global information such as time delta, GPS delta, and speed, because
          photobombs are more likely when taking images of several animals in a small time period.
        ...
        Thus, the chosen features correspond with those that intuitively make sense.
        This suggests that the major problem with the classifier is simply the amount of training data.

        We gain further insight into the weak performance of the photobomb-classifier by considering the failure
          cases illustrated in \cref{fig:PBFailures}.
        In each of these examples we observe that the classifier is either confused or it does not have enough
          information to label a pair as a photobomb.
        Furthermore, the classifier was able to find several new photobomb pairs that were not previously
          labeled.
        Examples of these errors in the groundtruth data are illustrated in~\cref{fig:PBLabelErrors}.
        Discovering these photobomb cases with high accuracy indicates that the photobomb-state classifier is
          actually stronger than our measurements suggest.
        Furthermore, this indicates that the learning algorithm encountered unnecessary difficulties because it
          was encouraged to incorrectly classify these errors.
        %By reviewing and relabeling these cases we 
        %Furthermore, because we know that each dataset contains photobombs that are not labeled, and hence were
        %  in the training data labeled not-photobomb increasing the difficulty for the learning algorithm.
        ...
        %This weak performance can be explained by considering several failure cases. 
