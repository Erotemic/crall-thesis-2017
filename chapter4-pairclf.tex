...
This is done by considering the importance of each feature dimension as measured using the using the ``mean
  decrease impurity'' (MDI)~\cite{louppe2014understanding}.
...
\section{Pairwise classification experiments}\label{sec:pairexpt}
    ...
    Our first test will compare the ability of each algorithm to predict if a pair is positive or not.
    We will look at the distribution of positive and non-positive scores as well as the ROC curves.
    The second test will compare the original LNBNN ranking against a re-ranking using the positive
      probabilities.
    %We will 
