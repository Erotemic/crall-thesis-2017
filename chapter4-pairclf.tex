...
%In the final pairwise feature vector, not all of the dimensions may be informative.
%It is useful to prune these uninformative dimensions, when using random forest learning.
%Too many feature dimensions can cause an otherwise informative dimension to be overlooked.

%Because we are using a random forest as the learning algorithm, an excessive number of
%feature dimensions correlation between feature dimensions
...
%The importance of each feature dimension is measured using the ``mean decrease
%  impurity'' (MDI)~\cite{louppe2014understanding}.
