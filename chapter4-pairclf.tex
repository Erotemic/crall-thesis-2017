...
This is because we noticed that some positive annotation pairs had all of their correspondences filtered by the
  ratio test.
However, when we increased the ratio threshold, the overall classification performance decreased.
Therefore, we included both small and large thresholds to allow the classifier to have access to both types of
  information.
...
So, there is a cost to using multiple values of the ratio test, first in the number of times spatial verification
  must be run and second in the resulting size of the feature vector.
...
\section{Learning the match-state classifier}\label{sec:learnclf}
    ...
    We included all pairs explicitly labeled as incomparable because there is only a few such examples.
...
\section{Pairwise classification experiments}\label{sec:pairexpt}
    ...
    \subsection{Matching state}
        ...
        In addition to demonstrating that the pairwise classifier outperforms LNBNN scores in this classification
          task, these ROC curves also demonstrates that an operating point can be chosen to automatically classify
          a significant number of positive pairs while making very few false positives.
        ...
        We find that reducing the number of feature dimensions to $25$ for plains and $61$ for Grévy's produces
          the best results, increasing the MCC by $.0155$ for plains and $.0048$ for Grévy's.
        ...
        In the last case, only a few distinctive matches were made, which suggests that procedure that
          establishes feature correspondences should be improved.
    ...
    \subsection{Photobomb state}
        ...
        Initially when performing this experiment, we classified a pair as a photobomb if its probability was
          greater than $0.5$.
        This caused us to measure an MCC of $0.0$ for the Grévy's zebra dataset because there were not enough
          training examples for the random forest to learn strong probabilities.
        Therefore, for each dataset we choose a classification threshold to maximize the MCC as illustrated in
          \cref{fig:PBThreshMCC}.
        Using this adjusted threshold, the overall performance is given in the classification confusion matrix
          illustrated in \cref{tbl:ConfusionPhotobombII} and evaluation metrics are given in
          \cref{tbl:EvalMetricsPhotobombII}.
        ...
        By choosing the appropriate threshold we achieve an MCC of $0.34$ for plains zebras and $0.40$ for
          Grévy's zebras, which indicates that each photobomb-state classifier has weak but significant predictive
          power.
        While the MCCs are not overwhelmingly strong, they indicate that the photobomb-classifier is learning,
          and could likely be improved with more training data.

        The conclusion that the photobomb-state classifier is learning is supported by investigating the top $10$
          most important features.
        These important features --- illustrated in~\cref{tbl:ImportantPBFeat} --- correspond with those that
          intuitively make sense.
        We find that photobomb classifier relies on global information such as time delta, GPS delta, and speed,
          because photobombs are more likely when taking images of several animals in a small time period.
        ...
        We gain further insight into the photobomb-state classifier by considering examples of failure cases,
          which are illustrated in \cref{fig:PBFailures}.
    ...
    \subsection{Classifier experiment conclusions}
        ...
        When making automatic decisions, these probabilites have several advantages over the LNBNN scores
          from~\cref{chap:ranking}.
        Not only do they have more predictive power, they are interpretable, and they always range between $0$
          and $1$.

        We will use these classifiers to make automatic decisions about pairs of anotations where the match-state
