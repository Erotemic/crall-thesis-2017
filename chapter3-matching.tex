...
\section{Experiments}\label{sec:rankexpt}
    ...
    \subsection{Invariance experiment}\label{sub:exptinvar} %
        ...
        Interestingly, affine invariance results in worse performance when QRH is on, but if the QRH is off then
    ...
    \subsection{K experiment}\label{sub:exptk}  
        ...
        overall annotations in the database, then it might be beneficial to search deeper into all the database
        ...
        The difference is that we use the extra confuser annotations to vary the total number of annotations in
          the database.
        ...
        The results of this experiment are illustrated in ~\cref{fig:KExptA,fig:KExptB}. Similar to the previous
        ...
        but magnitude of the decrease is between $1\percent$ and $3\percent$. Interestingly, the optimal choice of
    ...
    \subsection{Failure cases}\label{sub:exptfail}  
        ...
        \FloatBarrier{}
          \paragraph{Non-primary correspondences}
            ...
            Photobombs are caused by correct correspondences to a non-primary animal (seen either in the
              foreground or background) in an annotation.
            ...
            For plains and Grévy's zebras, most scenery matches are eliminated using the foregroundness measure,
              but the problem remains in databases without a trained foregroundness estimator.
    ...
    \subsection{Experimental conclusions}\label{sub:exptsum}  
        ...
        LNBNN does not quantize descriptor and therefore it is able to distinguish more subtle descriptor
          details.
        Because LNBNN does not require an expensive pre-training phase, it makes it ideal to rank the databases
          on the scales considered in this thesis.
