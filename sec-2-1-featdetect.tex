\section{Image feature detection}\label{sec:featuredetect}
    ...
    works related to detecting features in an image, and~\cref{sec:featuredescribe} will discuss how detected
    ...
    Hessian~\cite{beaudet_rotationally_1978, lindeberg_shapeadapted_1994} or difference of
    ...
    methods~\cite{buoncompagni_saliencybased_2015} and superpixel based methods~\cite{ren_learning_2003,
    ...
    recognition, use specialized keypoint detectors~\cite{dantone_realtime_2012, berg_tomvspete_2012}. There
   ...
   \subsection{Scale-space}
        ...
        scales~\cite{lindeberg_scalespace_1993}. The same point on an object seen close up appears quite different
        ...
        orthogonal directions~\cite{lindeberg_scalespace_1993}. Using the definition of an image at a single scale
       ...
       \paragraph{Gaussian pyramid}
           ...
           \newcommand{\downsamp}[2]{#1[::\tightpad#2,::\tightpad#2]}
            ...
            pyramid. An scale-space pyramid consists of $L$ levels. Each level covers an octave. Starting from the
    ...
    \subsection{Hessian keypoint detection}
        ...
        scale~\cite{beaudet_rotationally_1978, lindeberg_shapeadapted_1994}. The Hessian detector can qualitatively
        ...
        \paragraph{Hessian response}
            ...
            response is scale normalized by multiplying with $\sigma^2$ (See~\cite{lindeberg_feature_1998} for more
            details about the choice of this normalization factor). The extrema of this space defines a set of
        ...
        \paragraph{Sub-pixel and sub-scale localization}
            ...
            in~\cite{ke_pcasift_2004}, where descriptors were computed on the normalized vectors of patch gradients
    ...
    \subsection{Affine adaptation}
        ...
        matrix~\cite{lindeberg_shapeadapted_1997,baumberg_reliable_2000,mikolajczyk_comparison_2005}. The affine
        ...
        downwards --- \ie{} in the direction of the ``gravity vector''~\cite{perdoch_efficient_2009}. Assuming the
        gravity vector removes a dimension of invariance. To allow for the specification of keypoint orientation,
        the keypoint representation can be extended with a parameter $\theta$ that defaults to $0$.

    \subsection{Orientation adaptation}

        The keypoint orientation is defined using the parameter $\theta$. Without orientation adaptation a keypoint
        can be assumed to be aligned with the ``gravity vector'' --- \ie{}
        $\theta=0$~\cite{perdoch_efficient_2009}. Otherwise, an orientation must be computed. A common method for
        determining a keypoint's orientation is to use the dominant gradient orientation.
    ...
    \subsection{Discussion --- detector and invariance choices}
        ...
        orientation invariance assume patches can freely rotate, when in fact they may be constrained by the
        orientation of surrounding patches~\cite{lowe_distinctive_2004}. On the other side extreme is the ``gravity
        vector''~\cite{perdoch_efficient_2009}, which globally enforces all keypoint to have a downward
        orientation. This may be a safe assumption when working with image features from rigid objects taken in an
        upright position, but may not be correct when dealing with non-rigid objects like zebras.
