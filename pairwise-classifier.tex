...
To evaluate the pairwise classifier we choose a sample dataset as detailed in
  \Cref{sec:learnclf}, except we use stratified k-fold cross validation to split
  the dataset into several training and testing sets.
In our cross validation we add the constraint that edges within the same name
  and edges between the same names must be grouped in the same set.
This will ensure that our results generalize to new individuals.
...
\Cref{fig:WordCloud} and the top $5$ are given in \cref{tbl:ImportantFeat}.

We measure binary classification performance in a one-vs-rest manner focusing
  on positive-vs-rest.
...
TODO: SHOW:
For match state:
    * ROC Curves for one-vs-rest positive pairwise classifier
    * Match State Confusion Matrices for 3 cases
    * Match State Failure Cases
    * Word cloud of feature importnace
    * top 5 feature importance 

For photobomb state:
    * Photobomb ROC curves
    * Word cloud of feature importnace
