...
In this chapter we consider the problem of verifying whether or not two annotation are the same or different.
By addressing this problem we improve upon the ranking algorithm from \cref{chap:ranking} --- that ranks the
...
Each of these mutually exclusive ``match-states'' are illustrated in \cref{fig:MatchStateExample}.
...
A final concern that is investigated in this chapter is the issue of image challenges that may confound the
...
%it may be reasonable to expand the match-state classifier to learn this subtlety.
...
\begin{minted}[gobble=4]{python}
    OrderedDict([('global(qual_min)',    3),
                 ...
                 ('std(ratio)',          0.09)])
\end{minted}
\caption[\caplbl{PairFeatVec}A pairwise feature vector]{\caplbl{PairFeatVec} Example of a small pairwise feature
  vector containing local and global information.
...
This results in a final descriptor vector that can be quite large (thousands of dimensions) and some of the
  dimensions might not be useful.
Therefore, we prune the feature vector using only the most useful of the proposed dimensions.
This is done by considering the importance of each feature dimension as reported by the random forest classifier.
...
%The training data for the pairwise classifier is a set of annotation pairs and a set of corresponding labels.
%To train the classifier we follow the methodology of selecting hard negatives and moderate
%  positives~\cite{shi_embedding_2016}.
...
%Because our current datasets only contain a small number of explicitly labeled incomparable cases, we include all
%  such pairs.
...
\section{Summary of pairwise classification}\label{sec:pairconclusion}
    ...
    %\item The probabilities from the match-state classifier can be used to re-rank results of the ranking
    %  algorithm.
    %Because the classifier was trained using difficult training examples, it is able to correctly re-rank the
    %  case where the correct match was ranked highly, but not in rank $1$.

    %\item The probabilities can be used to make automatic decisions about the highest confidence results.
    %This is possible because the output from the pairwise algorithm is interpretable, ranging between $0$ and
    %  $1$.
    %We have found that these probabilities are well calibrated.
    %A validation dataset can be used to select a threshold where the false positive rate is sufficiently low
    %  (even $0$).
    %Because the positive cases are well separated from the negative and incomparable cases, a significant number
    %  of automatic reviews is possible.

    %\item Because the classifier outputs probabilities about pairs independent of their rank, we can address the
    %  case where a correct positive match does not exist, and automatically reject negative results at rank $1$.
    %\end{itemln}
